...
<i>👇Телефон рақам тугмасини босинг.</i> 
   `, {
    ...
    if(res_status.status){
        ...
        // success login
        await ctx.reply(`
<i>🎉 ${res_status.data.full_name} сиз тизимга муваффақиятли равишда кирдингиз!</i>      
        `, {
            parse_mode:"HTML",
...
<i>Агар сиз буни хато деб ҳисобласангиз қуйидаги  маъсул ходимга алоқага чиқинг!</i>  
...
async function station_details_conversation(conversation, ctx) {
    ...
    let group_btn = new Keyboard()
        ...
        .text("🔙 Орқага")
        .resized()
    await ctx.reply("🚏 Станция бўйича", {
        parse_mode:"HTML",
        reply_markup: group_btn,
    })
}
...
async function duration_time_conversation(conversation, ctx) {
    ...
    if(button_label_list.length ===0){
        button_label_list.push("Станцияда турган вагонлар йўқ")
    }
...
🏁 Чиқган станция: <b>${msg.first_station?.station_name_ru}</b>     
🏳️ Жорий станция: <b>${msg.current_station?.station_name_ru} </b>     
⏰ Сарфланган вақт: <b>${msg.wait_time} кун</b>     
🏴 Бораётган станция: <b>${msg.last_station?.station_name_ru} </b>  
...
pm.hears("🔼 Кетаётган вагонлар", async (ctx)=>{
    ...
    let msg_template =  `
<b>📊 Станциядан кетаётган вагонларнинг ҳозирда турган станциялари бўйича ҳисоботи</b>
    `
    group_station.forEach((item, index)=>{
        msg_template =msg_template + `
 ${item.name}: <b>${item.count} </b> та вагон`;
    })

    msg_template = msg_template +`

<i>📑 Умумий вагонлар сони</i>: <b>${res_data.data.amount}</b> та вагон



<i>👇Батафсил маълумотларни кўриш учун керакли станцияни танланг</i>    
    `
...
pm.hears("⏹ Турган вагонлар", async (ctx)=>{
    ...
    let group_station = res_data.data.group_station;
     ...
     ctx.session.session_db.group_station_list = group_station;
    let msg_template =  `
<b>📊 Станцияда турган вагонларнинг бораётган станциялари бўйича ҳисоботи</b>
    `
    group_station.forEach((item, index)=>{
        msg_template =msg_template + `
 ${item.name}: <b>${item.count} </b> та вагон`;
...
pm.hears("🔽 Келаётган вагонлар", async (ctx)=>{
    ...
    ctx.session.session_db.group_station_list = group_station;
    let msg_template =  `
<b>📊 Станцияга келаётган вагонларнинг ҳозирда турган станциялари бўйича ҳисоботи</b>
    `
    group_station.forEach((item, index)=>{
        msg_template =msg_template + `
...
pm.hears("🚏 Станция бўйича", async (ctx)=>{
...
pm.hears("1 кундан - 5 кунгача 🟢", async (ctx)=>{
    ...
    ctx.session.session_db.group_station_list = group_station;
    let msg_template =  `
<b>🟢 1 кундан - 5 кунгача станцияда турган вагонларнинг бораётган станциялари бўйича ҳисоботи</b>
    `
    group_station.forEach((item, index)=>{
        msg_template =msg_template + `
 ${item.name}: <b>${item.count}</b>`;
...
pm.hears("6 кундан - 10 кунгача 🟡", async (ctx)=>{
    ...
    ctx.session.session_db.group_station_list = group_station;
    let msg_template =  `
<b>⛔️ 6 кундан - 10 кунгача станцияда турган вагонларнинг бораётган станциялари бўйича ҳисоботи</b>
    `
    group_station.forEach((item, index)=>{
        msg_template =msg_template + `
 ${item.name}: <b>${item.count}</b>`;
...
pm.hears("11 кундан кўп 🔴", async (ctx)=>{
    ...
    ctx.session.session_db.group_station_list = group_station;
    let msg_template =  `
<b>⛔️ 11 кундан кўп станцияда турган вагонларнинг бораётган станциялари бўйича ҳисоботи</b>
    `
    group_station.forEach((item, index)=>{
        msg_template =msg_template + `
 ${item.name}: <b>${item.count}</b>`;
...
pm.hears("🗞 Вагон буюртмалар", async (ctx)=>{
    ...
    if(res_data.status){
        ...
        worksheet .addRow(['Станция номи', 'Вагон тури', 'Вагон сони', "Қўшимча изоҳ", "Вақт"]);
...
pm.hears("👤 Маълумотларим", async (ctx)=>{
    ...
    if(res_data.status){
        await ctx.reply(`
<b>👤 Профил маълумотлар</b>  

🚏 Станция: <b>${res_data.data?.organization?.station_name_ru}</b>
...
<i>✍️ Ботдан фойдаланиш вақтида қандайдир хатоликни сезсангиз (ва ботни яхшилаш учун таклифлариз булса) мутахасисларимизга хабар беришингизни сўраймиз!</i>
...
pm.hears("🔍 Вагон қидирув", async (ctx)=>{
    ...
    await ctx.reply(`
<b>🔍 Вагонни рақам орқали қидирув</b>
